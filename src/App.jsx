...
const App = () => {
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: 'Hello! I\'m your AI assistant. Ask me anything about programming, JavaScript, or any other topic you\'d like help with.'
    }
  ]);
      ...
      if (index % 3 === 2) {
        ...
        return (
          <div key={index} className="prose prose-sm max-w-none">
            {part.split('\n').map((line, lineIndex) => {
              if (line.startsWith('## ')) {
                return <h3 key={lineIndex} className="text-lg font-semibold mt-4 mb-2 text-gray-800">{line.slice(3)}</h3>;
              }
              if (line.startsWith('**') && line.endsWith('**')) {
                return <p key={lineIndex} className="font-semibold text-gray-700 my-1">{line.slice(2, -2)}</p>;
              }
                ...
                return (
                  <p key={lineIndex} className="my-1">
                    {inlineCodeParts.map((codePart, codeIndex) =>
                      codeIndex % 2 === 1 ?
                        <code key={codeIndex} className="bg-gray-100 px-1 py-0.5 rounded text-sm font-mono text-gray-800">{codePart}</code> :
                        codePart
                    )}
                  </p>
                );
              }
              return line.trim() ? <p key={lineIndex} className="my-1 leading-relaxed">{line}</p> : <br key={lineIndex} />;
            })}
          </div>
        );
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center p-2 sm:p-4">
      <Card className="
        ...
        <CardHeader className="
          border-b border-slate-200/60 bg-white/50 px-3 py-3
          sm:px-6 sm:py-4
        ">
        ...
        <CardContent className="flex flex-col flex-1 p-0 relative overflow-hidden">
          <ScrollArea
            className="
              flex-1 p-2 sm:p-4 overflow-y-auto max-h-[60vh] sm:max-h-none
            "
            ref={scrollAreaRef}
          >
          ...
          <div className="border-t border-slate-200/60 p-2 sm:p-4 bg-white/50">
            <div className="flex flex-col gap-2 sm:flex-row sm:gap-3 w-full">
              <Input
                ...
                className="
                  flex-1 rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500/20
                  min-w-0 px-3 py-2 text-sm sm:text-base
                "
                disabled={loading}
              />
              <Button
                ...
                className="
                  rounded-xl bg-blue-600 hover:bg-blue-700 disabled:opacity-50 px-4 
                  w-full sm:w-auto 
                "
